 ...
 */
function pickClient(
  attendees: { email?: string | null; self?: boolean | null; organizer?: boolean | null; responseStatus?: string | null; displayName?: string | null }[] | undefined,
  ...
  const excluded = new Set(
    [ownerEmail, organizerEmail].filter(Boolean).map((e) => e!.toLowerCase())
  );
...
export async function syncCalendarEvents(): Promise<{
  ...
  for (const event of events) {
    ...
    const clientRsvp = (clientAttendee.responseStatus ??
